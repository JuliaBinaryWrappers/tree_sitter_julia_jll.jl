...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import tree_sitter_julia_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("52be201e-a5e9-5cfe-8bf2-2dc4b062171c"))
platforms = [Pkg.Artifacts.unpack_platform(e, "tree_sitter_julia", artifacts_toml) for e in artifacts["tree_sitter_julia"]]
...
if best_platform === nothing
    @debug("Unable to load tree_sitter_julia; unsupported platform $(triplet(platform_key_abi()))")
else
